...
ggsave(here("03-outputs", "rooftop_pv_installation_size_state.png"), width = 400, height = 200, units = "mm")
```
...
ggsave(here("03-outputs", "rooftop_pv_typical_cost_size_nsw.png"))
```
...
rooftop_pv_rebate_df <- annual_kw_cost_df %>%
  rowwise() %>%
  mutate(
...
# Construct solar rebate df for the stacked bar plot (percentages)
rooftop_pv_rebate_df_long <- rooftop_pv_rebate_df %>%
...
# Create the plot
ggplot(rooftop_pv_rebate_df_long, aes(x = factor(Year), y = Value, fill = CostComponent)) +
  geom_col(position = "stack") +  # Stacked bars
  geom_line(data = rooftop_pv_rebate_df, 
            aes(x = factor(Year), y = OutOfPocket / household_cost_scale, group = 1, color = "Household Out-of-Pocket ($)"), 
            size = 1, inherit.aes = FALSE) +
  geom_point(data = rooftop_pv_rebate_df, 
  ...
  scale_color_manual(
    values = c("Household Out-of-Pocket ($)" = "gray30"),
...
ggsave(here("03-outputs", "solar_rebate_vs_pocket.png"))
```
...
ggsave(here("03-outputs", "rooftop_pv_fincial_return.png"), plot = combined_plot, width = 25, height = 20, units = "cm")
```
...
ggsave(here("03-outputs", "solar_bill_savings_kwh_output_dnsps.png"), width = 25, height = 20, units = "cm" )
...
ggplot(state_weighted_savings, aes(x = State, y = weighted_avg_savings, fill = pv_size)) +
  ...
  theme(
    ...
    plot.title = element_text(size = 16, face = "bold", margin = margin(t = 10, b = 5)),
    plot.subtitle = element_text(size = 12),
    legend.text = element_text(size = 12),
    legend.title = element_text(size = 12),
    panel.grid = element_blank(),
...
ggsave(here("03-outputs", "solar_bill_savings_kwh_output_state.png"), width = 25, height = 20, units = "cm" )
```
